...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1
                       FROM pg_trigger
                       WHERE tgname = 'set_timestamp' AND tgrelid = 'organizations'::regclass) THEN
...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_timestamp' AND tgrelid = 'users'::regclass) THEN
            CREATE TRIGGER set_timestamp
                BEFORE UPDATE
                ON users
                FOR EACH ROW
...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_timestamp' AND tgrelid = 'projects'::regclass) THEN
            CREATE TRIGGER set_timestamp
                BEFORE UPDATE
                ON projects
                FOR EACH ROW
...
$$
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_timestamp' AND tgrelid = 'tasks'::regclass) THEN
            CREATE TRIGGER set_timestamp
                BEFORE UPDATE
                ON tasks
                FOR EACH ROW
